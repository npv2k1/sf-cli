...
async fn test_watch_encrypt_basic() {
    ...
    assert!(test_watcher
        .config()
        .should_process_by_operation(&test_file));
...
async fn test_watch_extension_filtering() {
    ...
    assert!(watcher
        .config()
        .should_process_file(&watch_dir.join("test.txt")));
    assert!(watcher
        .config()
        .should_process_file(&watch_dir.join("document.doc")));

    // Test files that should not be processed
    assert!(!watcher
        .config()
        .should_process_file(&watch_dir.join("image.jpg")));
    assert!(!watcher
        .config()
        .should_process_file(&watch_dir.join("video.mp4")));
}
